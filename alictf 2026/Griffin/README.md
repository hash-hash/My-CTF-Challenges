...
最终可能有一到两个候选项并且有一个较小的位移（大概 5 左右），需要进行枚举。通过求根，可以得到 $\text{FLAG}\ mod\ p+1$ 的结果，其中 $p$ 大概 170 比特左右。注意到 FLAG 的格式，只有 32 个 hex 字符未知，简单的构造是按照 $256^i$ 构造背包格，容易发现这个对于数字加小写字母的组合并不好 balance，可以通过构造 $16^i$ 的格来获得更好的 balance 效果。
